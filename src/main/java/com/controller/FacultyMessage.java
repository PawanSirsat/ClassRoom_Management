...
@WebServlet("/sendMessageFaculty")
public class FacultyMessage extends HttpServlet
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		int loggedInFacultyId = 2;
		int loggedInStudentId = 2;
		...
		if (token != null && AuthenticationPoint.validateToken(token))
		{
			...
			{
				request.getRequestDispatcher("Faculty_Message_Module.jsp?success=true").forward(request, response);
			} else
			{
				request.getRequestDispatcher("Faculty_Message_Module.jsp?success=false").forward(request, response);
			}
